...
- `/audit` 当前按研究页真实口径做审计，覆盖 `COIN-M / USDT-M + 4小时 / 8小时 / 日线 / 3日线 / 周线`
...
- `/research-2` 当前已支持 `COIN-M / USDT-M + 周线 / 3日线 / 日线 / 4小时 / 8小时 + 同名单多币种`，状态顺序为 `大牛 / 小牛 / 震荡牛 / 震荡灰 / 震荡熊 / 小熊 / 大熊`
- `/research-2` 当前采用“连续特征分段 -> 段级命名 -> 相邻同标签合并”的结构，页面展示价格、布林线、EMA、SMA、RSI、周费率、ADX、BBW 分位、Return Z、日均成交量
- `/research-2` 当前会按市场、周期与币种读取本地完整历史；USDT-M 当前只覆盖与 COIN-M 同名单的币种
...
- 前端页结构
  - `/audit` 已改成研究数据审计页，当前直接审 `COIN-M / USDT-M + 4h / 8h / day / 3day / week`
