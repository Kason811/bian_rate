...
def load_volume_summary(workbook: Path) -> pd.DataFrame:
...
def main() -> None:
    ...
    volume_summary = load_volume_summary(workbook)
    ...
    with sqlite_connection() as conn:
        ...
        try:
            ...
            high_liquidity_symbols = [
                ...
                for row in volume_summary.itertuples(index=False)
            ...
            finalize_collector_run(
                ...
                notes=f"imported funding from {workbook.name}; skipped volume history because workbook only contains 30d averages",
            )
