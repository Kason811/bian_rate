...
def fetch_coinm_day_klines(base_asset: str, start_ms: int) -> list[list[object]]:
...
def fetch_usdtm_day_klines(symbol: str, start_ms: int) -> list[list[object]]:
  rows: list[list[object]] = []
  cursor = start_ms
  now_ms = int(time.time() * 1000)
  while True:
...
def write_cache(market: str, base_asset: str, rows: list[list[object]]) -> Path:
  output_dir = ROOT / "web" / "lib" / "research-klines" / market / "day"
...
def main() -> None:
  ...
  for base_asset in coinm_targets:
    rows = fetch_coinm_day_klines(base_asset, coinm_onboard.get(base_asset, 0))
    output_path = write_cache("coinm", base_asset, rows)
    print(f"coinm {base_asset}: {len(rows)} day klines -> {output_path}", flush=True)

  usdtm_onboard = load_usdtm_onboard_dates()
  usdtm_targets = load_usdtm_targets()
  for base_asset, symbol in usdtm_targets:
    rows = fetch_usdtm_day_klines(symbol, usdtm_onboard.get(symbol, 0))
    output_path = write_cache("usdtm", base_asset, rows)
    print(f"usdtm {base_asset}: {len(rows)} day klines -> {output_path}", flush=True)
