...
const MARKET = "usdtm";
const TIMEFRAME = "3day";
const WINDOW_BARS = 365;
...
function directionToChinese(direction) {
...
async function main() {
  ...
  for (const tuning of TUNINGS) {
    ...
    for (const symbol of symbols) {
      ...
      for (let segmentIndex = 1; segmentIndex < full.segments.length; segmentIndex += 1) {
        ...
        if (previousSegment.family !== "Sideways") continue;
        if (segment.family !== "Bull" && segment.family !== "Bear") continue;
        ...
        candidates.push({ signalIndex, expectedFamily: segment.family });
      }
  ...
  const payload = {
    ...
    mode: "realtime-window-rebuild-sideways-follow-trade",
    marketType: MARKET,
    timeframe: TIMEFRAME,
    symbols,
  ...
  const jsonPath = path.join(OUTPUT_DIR, "2026-04-07-usdtm-3day-sideways-follow-trade.json");
  const mdPath = path.join(OUTPUT_DIR, "2026-04-07-usdtm-3day-sideways-follow-trade.md");
