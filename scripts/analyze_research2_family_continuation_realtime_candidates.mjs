...
for (const tuning of TUNINGS) {
  for (const dataset of datasetSeeds) {
    ...
    for (let segmentIndex = 1; segmentIndex < full.segments.length; segmentIndex += 1) {
      ...
      const current = getRealtimeResearch(dataset, source, tuning, eventIndex);
      const next = getRealtimeResearch(dataset, source, tuning, eventIndex + 1);
...
const summary = {
  ...
  mode: "realtime-window-rebuild-candidates",
  candidateGeneration: "static full-sample family flips -> realtime window exact verification",
...
const jsonPath = path.join(OUTPUT_DIR, "2026-04-07-research2-family-continuation-realtime-candidates.json");
fs.writeFileSync(jsonPath, `${JSON.stringify(summary, null, 2)}\n`, "utf8");

const mdLines = [
  "# Research2 Family Continuation Realtime Candidates",
  "",
  `生成时间: ${summary.generatedAt}`,
  "说明: 先用静态全样本找最新区块刚变色的候选点，再按“实时重构图表”的窗口逻辑对候选点逐个精确验证。",
...
const mdPath = path.join(OUTPUT_DIR, "2026-04-07-research2-family-continuation-realtime-candidates.md");
